...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        try
        {
            ...
            // Open file in read mode and get file params to request url
            var file = new FileStream(filepath, FileMode.Open, FileAccess.Read);
            var fileParams = Utils.GetUploadFileParams(file);

            // Create request
            var request = new FileScanUploadURLRequest.Builder().WithProvider("reversinglabs").WithVerbose(true).WithRaw(true).WithTransferMethod(TransferMethod.PostURL).WithFileParams(fileParams).Build();
